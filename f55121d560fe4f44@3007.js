...
export default function define(runtime, observer) {
    ...
    const datafile = "./files/un_climate_data/UNdata_Export_updated.csv"; //"./files/original_stock_companies.csv"; "./files/naics_establishments.csv";
    const fileAttachments = new Map([[datafile, new URL(datafile, import.meta.url)]]);

    main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
    main.variable(observer()).define(["md"], function (md) {
        return (
            md`# Greenhouse Gas Emissions

Real world data from [data.un.org](http://data.un.org/explorer.aspx)`
        )
    });


    // DATA PARSER
    main.variable().define("data", ["d3", "FileAttachment"], async function (d3, FileAttachment) {
        return (
            d3.csvParse(await FileAttachment(datafile).text(), d3.autoType)
        )
    });
    ...
    main.variable(observer("title")).define("title", ["md"], function (md) {
        return (
            md`## Methane (CH4) Emissions

Methane (CH4) Emissions without Land Use, Land-Use Change and Forestry (LULUCF), in kilotonne CO2 equivalent`
        )
    });
...
But what are these keyframes? Data, derived from the source data!`
    ...
    main.variable().define(["data"], function (data) {
        return (
            data
        )
    });
    ...
    main.variable().define(["d3", "data"], function (d3, data) {
        return (
    ...
    main.variable().define(["md", "n"], function (md, n) {
        return (
    ...
    main.variable().define("n", function () {
        return (
            12
        )
    });
    ...
    main.variable().define("names", ["data"], function (data) {
        return (
    ...
    main.variable().define("datevalues", ["d3", "data"], function (d3, data) {
        return (
    ...
    main.variable().define(["md"], function (md) {
        return (
            md`Now we’re ready to compute the zero-based rank for each brand. The *rank* function below takes a *value* accessor function, retrieves each brand’s value, sorts the result by descending value, and then assigns rank. For any missing data—remember, turnover—we treat the brand’s value as zero.`
        )
    });
    main.variable().define("rank", ["names", "d3", "n"], function (names, d3, n) {
        return (
            function rank(value) {
    ...
    main.variable().define(["rank", "datevalues"], function (rank, datevalues) {
        return (
            rank(name => datevalues[0][1].get(name))
        )
    });
    main.variable().define(["md", "duration", "k"], function (md, duration, k) {
        return (
...
Try disabling interpolation by setting *k* to 1 below, then scroll up to see how this affects the animation.`
    ...
    main.variable().define("k", function () {
        return (
            10
        )
    });
    main.variable().define(["md", "tex"], function (md, tex) {
        return (
            md`Since our *rank* helper above takes a function, so we can use it to interpolate values [linearly](https://en.wikipedia.org/wiki/Linear_interpolation). If ${tex`a`} is the starting value and ${tex`b`} is the ending value, then we vary the parameter ${tex`t \in [0,1]`} to compute the interpolated value ${tex`a(1 - t) + bt`}.`
        )
    });
...
For example, between 2001 and 2002, Toyota enters the top 12 (moving from rank 14 to 12) while AT&T exits the top 12 (moving from rank 10 to 17). When animating Toyota’s entrance, we need to know the rank that it was coming from (14), and similarly when animating AT&T’s exit, we need to know the rank it is going to (17).`
    ...
    main.variable().define("nameframes", ["d3", "keyframes"], function (d3, keyframes) {
        return (
            d3.groups(keyframes.flatMap(([, data]) => data), d => d.name)
        )
    });
    main.variable().define("prev", ["nameframes", "d3"], function (nameframes, d3) {
        return (
            new Map(nameframes.flatMap(([, data]) => d3.pairs(data, (a, b) => [b, a])))
        )
    });
    main.variable().define("next", ["nameframes", "d3"], function (nameframes, d3) {
        return (
...
Enough with the data. Let’s draw!`
    ...
    main.variable().define("bars", ["n", "color", "y", "x", "prev", "next"], function (n, color, y, x, prev, next) {
        return (
            function bars(svg) {
...
As you might expect, the labels are implemented similarly to the bars.`
    ...
    main.variable().define("labels", ["n", "x", "prev", "y", "next", "textTween"], function (n, x, prev, y, next, textTween) {
        return (
            function labels(svg) {
    ...
    main.variable().define("textTween", ["d3", "formatNumber"], function (d3, formatNumber) {
        return (
    ...
    main.variable().define("formatNumber", ["d3"], function (d3) {
        return (
            d3.format(",d")
        )
    });
...
Our *x*-axis is top-anchored and slightly customized.`
    ...
    main.variable().define("axis", ["margin", "d3", "x", "width", "barSize", "n", "y"], function (margin, d3, x, width, barSize, n, y) {
        return (
            function axis(svg) {
...
The “ticker” in the bottom-right corner shows the current date.`
    ...
    main.variable().define("ticker", ["barSize", "width", "margin", "n", "formatDate", "keyframes"], function (barSize, width, margin, n, formatDate, keyframes) {
        return (
            function ticker(svg) {
                const now = svg.append("text")
                    //.style("font", `bold ${barSize}px var(--Helvetica)`)
                    .style("font", "bold 30px var(--Helvetica)")
                    .style("font-variant-numeric", "tabular-nums")
                    .attr("text-anchor", "end")
                    .attr("x", width - 6)
    ...
    main.variable().define("formatDate", ["d3"], function (d3) {
        return (
            d3.utcFormat("%Y")
        )
    });
...
The *x*-scale is linear. The chart mutates the domain as the animation runs.`
    ...
    main.variable().define("x", ["d3", "margin", "width"], function (d3, margin, width) {
        return (
    ...
    main.variable().define("y", ["d3", "n", "margin", "barSize"], function (d3, n, margin, barSize) {
        return (
            d3.scaleBand()
                .domain(d3.range(n + 1))
    ...
    main.variable().define("height", ["margin", "barSize", "n"], function (margin, barSize, n) {
        return (
            margin.top + barSize * n + margin.bottom
        )
    });
    main.variable().define("barSize", function () {
        return (
            48
        )
    });
